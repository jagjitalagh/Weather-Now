...
function App() {
  ...
  useEffect(() => {
    ...
    if (searchedLocation.length === 0) {
      ...
      fetch(
        `http://api.openweathermap.org/geo/1.0/direct?q=${searchedLocation}&limit=1&appid=${process.env.REACT_APP_VERY_PRIVATE_KEY}`
      )
        .then((response) => response.json())
        .then((data) => {
          if (data[0]?.local_names?.en !== undefined) {
            ...
            setSearchedLocation("");
            setFormError({
              searchedLocation: "You can search only for a city",
            });
          }
          setSearchDone(true);
        })
  ...
  useEffect(() => {
    if (searchDone) {
      fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&appid=${process.env.REACT_APP_VERY_PRIVATE_KEY}`
      )
        .then((response) => response.json())
        .then((data) => {
          setTodayWeather({
